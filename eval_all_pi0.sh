...
gpu_id=${1:-0}
# Same checkpoint slot as eval_all_ACT.sh; for pi0 this is the training step to load
ckpt_setting="30000"

# Per-task expert_data_num (same as eval_all_ACT.sh)
...
for task in "${tasks[@]}"; do
    ...
    bash eval_pi0.sh "$task" demo_clean "$ckpt_setting" "$clean_seeds_csv" "$gpu_id"

    echo "Evaluating $task | randomized | seeds: ${!rand_var}"
    bash eval_pi0.sh "$task" demo_randomized "$ckpt_setting" "$rand_seeds_csv" "$gpu_id"
done
